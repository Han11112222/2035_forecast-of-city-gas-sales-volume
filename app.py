...
def standardize_df(df_or_excel):
    ...
    # 2. Unnamed 컬럼 제거 (CSV 읽을 때 자주 발생)
...
def process_data_to_long(df, label_name, mapping):
    ...
    if '연' not in df.columns or '월' not in df.columns:
        return pd.DataFrame() # 연/월 없으면 실패
...
def render_analysis(df, unit):
    ...
    if df.empty:
        st.warning("분석할 데이터가 없습니다. (데이터 로드 실패)")
        return
...
def render_prediction(df, unit, start_year):
    ...
    for grp in groups:
        ...
        if method == "선형 회귀":
            ...
            try: model = LinearRegression(); model.fit(np.log(X.reshape(-1,1)), y)
            except: model = LinearRegression(); model.fit(X.reshape(-1,1), y) # 실패시 선형
            pred = model.predict(np.log(future_years))
        elif method == "지수 평활":
            pred = [y[-1]] * len(future_years) # 단순화
        else: # CAGR
...
def main():
    ...
    if mode.startswith("1"):
        ...
        if raw is not None:
            # 판매량은 시트가 여러개일 수 있음 (ExcelFile 체크)
            try:
            ...
            st.info("👈 좌측에서 '판매량' 파일을 업로드하거나 GitHub 연결을 확인하세요.")
    ...
    # 🟢 2. 공급량 예측 모드
        ...
        if raw_h is not None and raw_p is not None:
            st.sidebar.success("✅ 파일 2개 모두 연결됨")
            ...
            # 🐞 디버깅용: 만약 데이터가 비었다면 컬럼명 보여주기
            if df_final.empty:
                with st.expander("🚨 데이터가 비어있습니다. 컬럼명을 확인하세요!"):
                    st.write("과거실적 컬럼:", df_h.columns.tolist())
                    st.write("중기계획 컬럼:", df_p.columns.tolist())
                    st.write("사용 중인 매핑 키:", list(USE_COL_TO_GROUP.keys()))
        else:
            st.info("👈 좌측에서 '상품별공급량(과거)' 및 '사업계획(중기)' 파일을 업로드하세요.")
    ...
    if not df_final.empty:
        ...
        with st.sidebar:
            ...
            default_yrs = [y for y in all_years if y <= 2025] # 기본적으로 2025년 이하만 학습
