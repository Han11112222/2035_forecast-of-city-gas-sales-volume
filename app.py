...
# 2) 공급량용 매핑 (최신 수정 반영: 영업용 추가, 일반용1->업무용 이동)
MAPPING_SUPPLY_SPECIFIC = {
    ...
    # 나머지는 매핑 안함 -> 원래 컬럼명 사용 (주한미군, 열병합 등)
}
...
def make_long_data(df, label, mode='sales'):
    ...
    for col in df.columns:
        ...
        if mode == 'sales':
            ...
            # 매핑표에 있으면 쓰고, 없으면 원래 이름 사용 (나머지 그대로)
