...
# ─────────────────────────────────────────────────────────
# 🟢 2. 데이터 로드 (형식 무관 강제 로드)
# ─────────────────────────────────────────────────────────
@st.cache_data(ttl=600)
def load_data_robust(filename, uploaded_file=None):
    """
    확장자가 .xlsx여도 내용이 csv면 csv로 읽어내는 만능 로더
    """
    ...
    if Path(filename).exists():
        # 파일 경로 문자열 대신 파일 객체로 열어서 전달 (안전성 확보)
        with open(filename, 'rb') as f:
            content = io.BytesIO(f.read())
        return try_read_any_format(content)
...
def main():
    ...
    with st.sidebar:
        ...
        if main_cat == "1. 판매량 예측":
            ...
            except Exception as e: st.error(f"데이터 병합 오류: {e}")
