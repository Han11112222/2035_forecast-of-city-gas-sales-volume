...
def render_prediction_2035(long_df, unit_label, start_pred_year, train_years_selected, is_supply_mode=False):
    ...
    for grp in groups:
        ...
        # 판매량 모드(2026 시작) -> 26년 미만 모두 실적 취급 (이미 load단계에서 걸러짐)
    ...
    if mode.startswith("1"):
        if up_sales:
            ...
            if data:
                ...
                # 2. [핵심] 계획 데이터 정제 (과거 데이터 삭제)
                if df_p is not None:
                    df_p = clean_df(df_p)
                    if '연' in df_p.columns:
                        # 판매량의 경우 계획은 '미래'만 의미있으므로 2025년 이하는 삭제
                        df_p = df_p[df_p['연'] > 2025]
