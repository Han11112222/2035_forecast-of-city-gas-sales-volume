...
BRANCH_NAME = "main" # 혹시 master라면 여기를 master로 수정하세요
SALES_FILE_NAME = "판매량(계획_실적).xlsx"
TEMP_FILE_NAME = "기온_198001_202512.xlsx" # 업로드된 파일명 기준
...
def load_excel_from_github(filename):
    """깃허브 Raw 파일 로드 (디버깅 정보 포함)"""
    # URL 인코딩 (한글, 괄호 처리)
    encoded_filename = quote(filename)
    url = f"https://raw.githubusercontent.com/{GITHUB_USER}/{REPO_NAME}/{BRANCH_NAME}/{encoded_filename}"
    
    try:
        response = requests.get(url)
        response.raise_for_status() # 404 에러 시 여기서 멈춤
        return pd.ExcelFile(io.BytesIO(response.content), engine='openpyxl')
    except Exception as e:
        # 에러 발생 시 URL을 화면에 표시 (중요!)
        st.error(f"❌ 데이터 로드 실패! 아래 URL을 클릭해서 파일이 다운로드 되는지 확인해보세요.")
...
def load_temp_universal(file_obj=None):
    # 1. 깃허브 우선 시도 (기본값)
    if file_obj is None:
        try:
            url = f"https://raw.githubusercontent.com/{GITHUB_USER}/{REPO_NAME}/{BRANCH_NAME}/{quote(TEMP_FILE_NAME)}"
            res = requests.get(url)
            if res.status_code == 200:
                return preprocess_temp(pd.read_excel(io.BytesIO(res.content), engine='openpyxl'))
            # 엑셀 없으면 CSV 시도 (파일명이 기온.csv일 경우)
            url_csv = f"https://raw.githubusercontent.com/{GITHUB_USER}/{REPO_NAME}/{BRANCH_NAME}/{quote('기온.csv')}"
            res_csv = requests.get(url_csv)
            if res_csv.status_code == 200:
                try: df = pd.read_csv(io.BytesIO(res_csv.content), encoding='utf-8-sig')
                except: df = pd.read_csv(io.BytesIO(res_csv.content), encoding='cp949')
...
def render_analysis_dashboard(long_df, unit_label):
    ...
    if df_act.empty: st.warning("데이터 없음"); return
...
def render_prediction_2035(long_df, unit_label):
    ...
    df_act = long_df[(long_df['계획/실적'] == '실적') & (long_df['연'] <= 2025)].copy()
    if df_act.empty: st.warning("데이터 부족"); return
    ...
    st.info("**💡 Insight:** 2025년의 일시적 반등보다는 전체적인 추세를 기반으로 예측되었습니다.")
...
def render_household_analysis(long_df, df_temp, unit_label):
    ...
    if df_temp is None:
        st.error("🚨 기온 데이터가 로드되지 않았습니다. (GitHub/업로드 확인 필요)"); return
...
def main():
    ...
    xls_sales = load_excel_from_github(SALES_FILE_NAME)
    ...
    with st.sidebar:
        ...
        # 파일 업로드 (백업용)
        st.markdown("---")
        st.caption("데이터 파일 (GitHub 로드 실패 시 사용)")
        ...
        if is_loaded:
            try:
                ...
                train_years = st.multiselect(
                    "학습 연도", years_avail, default=years_avail, label_visibility="collapsed"
                )
    ...
    # ── 메인 화면 ──
    if not is_loaded:
        st.info("👈 GitHub 연결 실패 시, 좌측에서 파일을 직접 업로드해주세요.")
        return
