...
# 🟢 [추가] 형님이 지정하신 상품 정렬 순서 리스트
ORDER_LIST_DETAIL = [
    "취사용", "개별난방용", "중앙난방용",       # 가정용 그룹
    "영업용",                                 # 영업용
    "일반용(1)", "일반용(2)", "업무난방용", "냉난방용", "주한미군", # 업무용 그룹
    "산업용",                                 # 산업용
    "열병합용",                               # 열병합용
    "연료전지",                               # 연료전지
    "자가열전용",                             # 자가열전용
    "열전용설비용(주택외)",                    # 열전용설비용
    "수송용(CNG)", "수송용(BIO)"              # 수송용 그룹
]
...
def render_prediction_2035(long_df, unit_label, start_pred_year, train_years_selected, is_supply_mode):
    ...
    fig = px.line(df_res, x='연', y='값', color='그룹', line_dash='구분', markers=True, category_orders={'그룹': final_order})
    ...
    fig_stack = px.bar(df_res, x='연', y='값', color='그룹', title="연도별 용도 구성비", text_auto='.2s', category_orders={'그룹': final_order})
    ...
    with st.expander("📋 연도별 상세 데이터 확인"):
        ...
        # 🟢 [미미 추가] 표 컬럼 순서 강제 적용 (지정된 순서 + 소계)
        existing_cols = [c for c in final_order if c in piv.columns]
        piv = piv[existing_cols] # 순서 재배치
