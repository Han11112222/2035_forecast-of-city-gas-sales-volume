...
BRANCH_NAME = "main" # master라면 수정 필요
...
# ─────────────────────────────────────────────────────────
# 1. 데이터 로드 (URL 디버깅 기능 추가)
# ─────────────────────────────────────────────────────────
@st.cache_data(ttl=600)
def load_excel_from_github_debug(filename):
    # 한글 파일명 URL 인코딩
    encoded_name = quote(filename)
    url = f"https://raw.githubusercontent.com/{GITHUB_USER}/{REPO_NAME}/{BRANCH_NAME}/{encoded_name}"
    
    try:
        response = requests.get(url)
        response.raise_for_status() # 404 에러 시 예외 발생
        return pd.ExcelFile(io.BytesIO(response.content), engine='openpyxl'), url
    except Exception as e:
        return None, url # 실패 시 시도한 URL 반환
...
def render_analysis_dashboard(long_df, unit_label):
    ...
    selected_years = st.multiselect("연도 선택", all_years, default=all_years, key="graph_years", label_visibility="collapsed")
...
def render_prediction_2035(long_df, unit_label):
    ...
    st.info(f"ℹ️ **학습 구간:** {train_years} (좌측 '학습 연도'에서 변경 가능)")
...
def render_household_analysis(long_df, df_temp, unit_label):
    ...
    if df_temp is None: st.error("🚨 기온 데이터가 없습니다. 좌측에서 업로드해주세요."); return
...
def main():
    ...
    # 1. 깃허브 로드 시도 및 URL 확인
    xls_sales, tried_url = load_excel_from_github_debug(SALES_FILE_NAME)
    ...
    with st.sidebar:
        ...
        if is_loaded:
            ...
            st.caption(f"시도한 URL: {tried_url}")
            st.warning("⚠️ 파일이 없거나, Private 저장소입니다. 아래에서 직접 파일을 올려주세요.")
            
            # 실패 시 백업 업로더 표시
            uploaded_sales = st.file_uploader("판매량 파일(.xlsx)", type="xlsx")
            if uploaded_sales:
        ...
        # 🟢 [학습 기간 선택] - 데이터가 로드된 경우에만 표시 (형님 요청 사항!)
        if is_loaded:
            try:
                ...
                st.markdown("---")
                st.markdown("**📅 학습/분석 대상 연도 (2015~2025)**")
                st.caption("체크 해제 시 분석 및 예측 학습에서 제외됩니다.")
                
                # 🔴 형님이 찾으시던 그 버튼입니다!
                train_years = st.multiselect(
                    "학습 연도",
                    options=years_avail,
                    default=years_avail, # 기본: 전체 선택
