...
TEMP_FILE_NAME = "기온.csv" # 기온 파일 기본값
...
def render_prediction_2035(long_df, unit_label):
    ...
    st.info("""
    **💡 가정용 추세 분석 Insight:**
    2025년 판매량이 반등했음에도 향후 추세가 완만한 이유는 AI가 2025년의 급등을 일시적 현상으로 보고, 과거 5년의 전체적인 평균 기울기를 반영했기 때문입니다.
    """)
...
def main():
    ...
    with st.sidebar:
        ...
        if main_category == "1. 판매량 예측":
            # 형님 요청: "지금 보이는 모든 화면(실적, 예측, 가정용)을 1번에 넣기"
            sub_mode = st.radio("기능 선택", ["1) 실적분석", "2) 2035 예측", "3) 가정용 정밀 분석"])
        else:
            # 공급량 예측 (나중에 데이터 추가 시 활성화)
            sub_mode = st.radio("기능 선택", ["1) 실적분석", "2) 2035 예측", "3) 가정용 정밀 분석"])
    ...
    if sales_src == "☁️ GitHub":
        ...
        # 파일 업로드 로드
        if uploaded_sales: xls_sales = pd.ExcelFile(uploaded_sales, engine='openpyxl')
        
    if not xls_sales:
        st.info("👈 데이터가 연결되지 않았습니다. (GitHub 자동 연결 시도 중...)")
        # 깃허브 로드 실패 시 메시지
        if sales_src == "☁️ GitHub": st.error(f"GitHub에서 '{SALES_FILE_NAME}'을 찾을 수 없습니다.")
        return
    ...
    try:
        ...
        st.error("엑셀 시트명(계획_부피 등)을 확인해주세요.")
        return
    ...
    if main_category == "1. 판매량 예측":
        ...
        st.warning("🚧 [공급량 예측] 메뉴는 아직 준비 중입니다.")
        st.info("판매량 데이터를 기반으로 한 분석은 '1. 판매량 예측' 메뉴를 이용해주세요.")
        # (나중에 공급량 데이터가 생기면 여기에 로직 추가)
